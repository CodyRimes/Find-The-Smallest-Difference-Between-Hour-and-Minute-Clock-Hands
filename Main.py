...
def calculateSmallestDifferenceInDegreesBetweenMinuteAndHourHandOnTheClock(hourHandInputResponse:int, minuteHandInputResponse:int) -> int:
    ...
    #Now that we know how many degrees each hand moves per minute (The minute hand moves 6 degrees for every minute, and the hour hand moves 0.5 degrees for every minute), we can code it up

    #We will need to multiply the degree per minute by the amount of minutes we have received from user input
    PositionInDegreesMovedByMinuteHand = (360/60) * minuteHandInputResponse

    PositionInDegreesMovedByHourHand = (((360/60)/12) * hourHandInputResponse)

    print(f"The minute time for the minute hand is at:  {minuteHandInputResponse}. We have calculated the position it is in in degrees is: {PositionInDegreesMovedByMinuteHand}.")
    print(f"The hour time for the hour hand is at: {hourHandInputResponse}. We have calculated the position it is in in degrees is: {PositionInDegreesMovedByHourHand}.")

calculateSmallestDifferenceInDegreesBetweenMinuteAndHourHandOnTheClock(hourHandInputResponse, minuteHandInputResponse)